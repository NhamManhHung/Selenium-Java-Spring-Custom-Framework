...
public class UIElements extends ActionsBaseClass implements IUIElements {
    ...
    @Override
    public void checkBox(By by, boolean isCheck) {
        utilityClass.scrollHelper(driver, webDriverWait, applicationContext, by);
        ...
        if (isCheck && !isSelected) {
            checkbox.click();
            log.debug("Checkbox was unchecked. Clicked to check it.");
        } else if (!isCheck && isSelected) {
            checkbox.click();
            log.debug("Checkbox was checked. Clicked to uncheck it.");
        } else {
            log.debug("Checkbox state is already as expected (isCheck=" + isCheck + "). No action taken.");
        }
    }
    ...
    @Override
    public void selectByVisibleText(By select, String textForSelect) {
        utilityClass.scrollHelper(driver, webDriverWait, applicationContext, select);
        WebElement dropdownElement = driver.findElement(select);
        Select dropdown = new Select(dropdownElement);
        dropdown.selectByVisibleText(textForSelect);
        log.debug("Selected option '{}' from dropdown {}", textForSelect, select);
    }
